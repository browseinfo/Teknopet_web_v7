...
class Reports(openerpweb.Controller):
    ...
    def index(self, req, action, token):
        ...
        if report_ids:
            if file_name == "Tax Invoice":
                inv_obj = req.session.model('account.invoice')
                for ele_inv in report_ids:
                    inv = inv_obj.read(report_ids[0], ['code'], context)['code']
                    file_name =  file_name + ' ' +  str(inv)
            
            if file_name == "Quotation/SO Report":
                sale_obj = req.session.model('sale.order')
                for ele_sale in report_ids:
                    state = sale_obj.read(report_ids[0], ['state'], context)['state']
                    sale = sale_obj.read(report_ids[0], ['name'], context)['name']
                    if state in ['draft', 'sent']:
                        file_name =  'Quotation' + ' ' +  str(sale)
                    if state not in ['draft', 'sent']:
                        file_name =  'Sales Receipt' + ' ' +  str(sale)
...
#             if reportname == "custom_header_footer.report_saleorder_1":
...
#                     else :
#                         reportname = "Sales Receipt" + ' ' + str(sale_browse[0].name)
            # complete here report name customized
